...
- 401 Unauthorized : 클라이언트가 해당 리소스에 대한 인증이 필요함
  - 인증되지 않음
  - 401 오류 발생시 응답에 WWW-Authenticate 헤더와 함께 인증 바업을 설명
  - 오류메시지가 Unauthorized이지만 인증되지 않음임
...
5xx 에러는 진짜 서버에 문제가 있을 때 발생시켜야한다.  
크리티컬한 에러로 내야한다.  
20세 이상에게 제공하는 서비스인데 15세가 접근한다는것은 정상적인 로직이므로 서버에러로 내면 안된다.  
